...
public class Form2VideosMathematics extends AppCompatActivity {
    ...
    private void fetchVideos() {
        ...
        StorageReference storageRef = storage.getReference().child("/form2/sciences/mathematics/videos/");

        storageRef.listAll().addOnSuccessListener(listResult -> {
            List<Form2VideosMathematics.VideoItem> videos = new ArrayList<>();
            for (StorageReference item : listResult.getItems()) {
                item.getDownloadUrl().addOnSuccessListener(uri -> {
                    String name = item.getName();
                    String url = uri.toString();
                    videos.add(new Form2VideosMathematics.VideoItem(name, url));
