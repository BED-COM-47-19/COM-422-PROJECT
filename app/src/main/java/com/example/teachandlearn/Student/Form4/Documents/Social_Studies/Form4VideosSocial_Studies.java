...
public class Form4VideosSocial_Studies extends AppCompatActivity {
    ...
    private void fetchVideos() {
        ...
        StorageReference storageRef = storage.getReference().child("/form4/humanities/social_studies/videos/");

        storageRef.listAll().addOnSuccessListener(listResult -> {
            List<Form4VideosSocial_Studies.VideoItem> videos = new ArrayList<>();
            for (StorageReference item : listResult.getItems()) {
                item.getDownloadUrl().addOnSuccessListener(uri -> {
                    String name = item.getName();
                    String url = uri.toString();
                    videos.add(new Form4VideosSocial_Studies.VideoItem(name, url));
