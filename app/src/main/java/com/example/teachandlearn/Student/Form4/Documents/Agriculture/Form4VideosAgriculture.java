...
public class Form4VideosAgriculture extends AppCompatActivity {
    ...
    private void fetchVideos() {
        ...
        StorageReference storageRef = storage.getReference().child("/form4/sciences/agriculture/videos/");

        storageRef.listAll().addOnSuccessListener(listResult -> {
            List<Form4VideosAgriculture.VideoItem> videos = new ArrayList<>();
            for (StorageReference item : listResult.getItems()) {
                item.getDownloadUrl().addOnSuccessListener(uri -> {
                    String name = item.getName();
                    String url = uri.toString();
                    videos.add(new Form4VideosAgriculture.VideoItem(name, url));
