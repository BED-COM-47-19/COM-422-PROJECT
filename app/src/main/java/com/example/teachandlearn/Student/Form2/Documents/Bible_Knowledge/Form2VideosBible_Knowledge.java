...
public class Form2VideosBible_Knowledge extends AppCompatActivity {
    ...
    private void fetchVideos() {
        ...
        StorageReference storageRef = storage.getReference().child("/form2/sciences/bible_knowledge/videos/");

        storageRef.listAll().addOnSuccessListener(listResult -> {
            List<Form2VideosBible_Knowledge.VideoItem> videos = new ArrayList<>();
            for (StorageReference item : listResult.getItems()) {
                item.getDownloadUrl().addOnSuccessListener(uri -> {
                    String name = item.getName();
                    String url = uri.toString();
                    videos.add(new Form2VideosBible_Knowledge.VideoItem(name, url));
