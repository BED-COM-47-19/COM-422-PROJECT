...
public class Form4VideosMathematics extends AppCompatActivity {
    ...
    private void fetchVideos() {
        ...
        StorageReference storageRef = storage.getReference().child("/form4/sciences/mathematics/videos/");

        storageRef.listAll().addOnSuccessListener(listResult -> {
            List<Form4VideosMathematics.VideoItem> videos = new ArrayList<>();
            for (StorageReference item : listResult.getItems()) {
                item.getDownloadUrl().addOnSuccessListener(uri -> {
                    String name = item.getName();
                    String url = uri.toString();
                    videos.add(new Form4VideosMathematics.VideoItem(name, url));
