...
public class Form4VideosBiology extends AppCompatActivity {
    ...
    private void fetchVideos() {
        ...
        StorageReference storageRef = storage.getReference().child("/form4/sciences/biology/videos/");

        storageRef.listAll().addOnSuccessListener(listResult -> {
            List<Form4VideosBiology.VideoItem> videos = new ArrayList<>();
            for (StorageReference item : listResult.getItems()) {
                item.getDownloadUrl().addOnSuccessListener(uri -> {
                    String name = item.getName();
                    String url = uri.toString();
                    videos.add(new Form4VideosBiology.VideoItem(name, url));
