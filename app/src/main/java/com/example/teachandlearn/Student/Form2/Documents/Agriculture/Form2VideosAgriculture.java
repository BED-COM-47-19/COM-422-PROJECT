...
public class Form2VideosAgriculture extends AppCompatActivity {
    ...
    private void fetchVideos() {
        ...
        StorageReference storageRef = storage.getReference().child("/form2/sciences/agriculture/videos/");

        storageRef.listAll().addOnSuccessListener(listResult -> {
            List<Form2VideosAgriculture.VideoItem> videos = new ArrayList<>();
            for (StorageReference item : listResult.getItems()) {
                item.getDownloadUrl().addOnSuccessListener(uri -> {
                    String name = item.getName();
                    String url = uri.toString();
                    videos.add(new Form2VideosAgriculture.VideoItem(name, url));
